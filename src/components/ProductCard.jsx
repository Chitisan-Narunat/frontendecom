...
function FavHeart({ productId, defaultFav = false, token, onChanged, hasRemove = true }) {
  ...
  // สำคัญ: ถ้า parent อัปเดต isFavourite จากเซิร์ฟเวอร์ ให้ sync ลงปุ่มด้วย
  useEffect(() => {
    setIsFav(!!defaultFav);
  }, [defaultFav]);
  ...
  const toggleFav = async (e) => {
    ...
    // ถ้ายังไม่มีเส้นลบฝั่ง backend ให้กันไม่ให้สลับออก
    if (!hasRemove && isFav) return;

    const next = !isFav;       // สถานะหลังคลิก
    ...
    <div className="container text-center mx-auto">
      ...
      {chunk(items, 4).map((row, rowIdx) => (
        <div key={rowIdx} className="myContainer">
          {row.map((p) => {
            // ป้องกันเคสชื่อฟิลด์มาคนละเคสจาก backend
            ...
            return (
              <div key={p.productId ?? p.ProductId} onClick={() => onSelect?.(p)} className="containerBox relative">
                {/* ปุ่ม fav */}

...
function Profile() {
  ...
  const renderContent = () => {
    switch (activeIndex) {
      ...
      case 2:
        return <CardDetailTab />;
      case 3:
        return <FavouriteTab />;
      case 4:
        return <AddressTab />;
      case 5:
        return <LogoutTab onLogout={() => navigate("/login")} />;
      default:
        return null;
    }
  };
          ...
          <div className="bg-white w-[300px] h-[860px] mt-[66px] border-black">
            <ul className="mt-24">
              <ul className="flex flex-row space-x-6">
                <li>
                  <img
                    className="h-20 w-20 absolute top-[75px] left-3"
                    src={icon1}
                    alt=""
                  />
                </li>
                <li>
                  <h1 className="absolute top-[105px] font-bold left-24 text-xl text-black font-playfair">
                    {userName}
                  </h1>
                </li>
              </ul>
              <li>
                <button
                  ...
                  className={`w-full h-20 rounded-xl text-black font-serif text-lg hover:bg-gray-300 ${
                    activeIndex === 0 ? "bg-gray-300" : "bg-white"
                  }`}
                >
                  Profile
                </button>
              ...
              <li>
                <button
                  ...
                  className={`w-full h-20 rounded-xl text-black font-serif text-lg hover:bg-gray-300 ${
                    activeIndex === 6 ? "bg-gray-300" : "bg-white"
                  }`}
                >
                  Tracking
                </button>
              ...
              <li>
                <button
                  ...
                  className={`w-full h-20 rounded-xl text-black font-serif text-lg hover:bg-gray-300 ${
                    activeIndex === 1 ? "bg-gray-300" : "bg-white"
                  }`}
                >
                  Order History
                </button>
              ...
              <li>
                <button
                  ...
                  className={`w-full h-20 rounded-xl text-black font-serif text-lg hover:bg-gray-300 ${
                    activeIndex === 2 ? "bg-gray-300" : "bg-white"
                  }`}
                >
                  Card Detail
                </button>
              ...
              <li>
                <button
                  ...
                  className={`w-full h-20 rounded-xl text-black font-serif text-lg hover:bg-gray-300 ${
                    activeIndex === 3 ? "bg-gray-300" : "bg-white"
                  }`}
                >
                  My Favourite
                </button>
              ...
              <li>
                <button
                  ...
                  className={`w-full h-20 rounded-xl text-black font-serif text-lg hover:bg-gray-300 ${
                    activeIndex === 4 ? "bg-gray-300" : "bg-white"
                  }`}
                >
                  Address
                </button>
              ...
              <li>
                <button
                  ...
                  className={`w-full h-20 rounded-xl text-black font-serif text-lg hover:bg-gray-300 ${
                    activeIndex === 5 ? "bg-gray-300" : "bg-white"
                  }`}
                >
                  Log Out
                </button>
        ...
        >
          ...
          >
            ...
            <ul className="mt-[14px] space-y-5">
              {cart.items.length === 0 ? (
                <li className="text-center text-gray-500 py-10">ตะกร้าว่าง</li>
              ) : (
                    ...
                    <div className="w-full h-28 rounded-lg bg-gray-300 flex space-x-3">
                      <div className="w-24 h-20 flex justify-center items-start">
                        <img
                          ...
                          src={getImageById(items.productId)}
                          alt={items.name || ""}
                        />
                      </div>
                      <ul className="flex flex-col">
                        <li className="flex flex-row">
                          ...
                          <button
                            ...
                            onClick={() => removeItem(items.orderItemsId)}
                          >
                            <IoClose size={17} />
                          </button>
                        </li>
                        ...
                        <li className="flex flex-row space-x-3">
                          <h3 className="absolute right-24 text-xs font-semibold mt-1">
                            Quantity :
                          </h3>
                          <button
                            className="absolute right-20"
                            onClick={() =>
                              decQty(items.orderItemsId, items.qty)
                            }
                          >
                            ‹
                          </button>
                          <div className="bg-white w-5 h-4 absolute right-[53px] mt-[5px]">
                            <h2 className="flex justify-center items-center absolute  ml-[7px] text-xs">
                              {items.qty}
                            </h2>
                          </div>
                          <button
                            ...
                            onClick={() =>
                              incQty(items.orderItemsId, items.qty)
                            }
                          >
                            {" "}
                            ›{" "}
            ...
            <ul>
              <li>
                <h1 className="absolute bottom-8 text-xl font-bold">
              ...
              <li>
                <button
                  onClick={goToAddress}
                  className="w-20 h-10 absolute bottom-7 right-4 bg-gray-600 text-white rounded-xl shadow hover:bg-gray-800 transition"
                >
                  CheckOut
                </button>
              </li>
...
function ProfileTab() {
  ...
  async function refreshUserprofile() {
    try {
      const { data } = await api.get(`/Address/UserInfo`, {
        headers: auth(),
      });
      setUserprofile({
...
function OrderHistoryTab() {
  ...
  async function refreshOrderhistory() {
    const { data } = await api.get(`/Address/OrderHistory`, {
      headers: auth(),
    });
    setorderhistory({
    ...
    <div className="space-y-6">
      ...
      <div className="border border-gray-300 rounded-lg shadow hover:shadow-lg transition p-6 w-full h-fit text-center flex items-center justify-start bg-white flex-col space-y-4">
        {orderhistory.items.length === 0 ? (
          <p className="text-gray-500 items-center justify-center flex">
            ยังไม่มีรายการ
          </p>
        ) : (
          orderhistory.items.map((item) => (
            <div
              key={item.addressId}
              className="bg-gray-200 w-full h-[100px] rounded-lg text-left p-3"
            >
              <h1 className="font-medium">Date: {item.orderDate}</h1>
              <h1 className="font-medium">Price: ฿{item.actualPrice}</h1>
              <h1 className="font-medium">Status: {item.status}</h1>
            </div>
          ))
        )}
      </div>
  ...
  return (
    <main>
            ...
            address.items.map((item) => (
              ...
              >
                <button
                  ...
                  className="absolute right-[87px] -mt-2"
                >
                  x
                </button>
                ...
                <h1 className="font-medium text-sm mt-[0.5px]">
                  {item.name}, {item.district}, {item.province}{" "}
                  {item.postalCode}
                </h1>
                ...
                <button
                  className="text-xs ml-[990px] rounded-lg bg-gray-300 w-10 -mt-1 absolute hover:bg-gray-400"
      ...
      {isAddressOpen && (
        ...
        >
          <div
            className="bg-white rounded-lg shadow-lg p-6 w-[800px] h-fit relative"
            role="dialog"
            aria-modal="true"
          ...
          >
            ...
            >
              Edit Address #{editform.AddressId}
            </h2>
            <form
              onSubmit={handleSave}
            ...
            >
              ...
              <input
                ...
                onChange={(e) => onChangeField("AddressName", e.target.value)}
                required
              />
              <input
                type="text"
              ...
              />
              <button
                className="w-[180px] bg-gray-600 text-white py-2 rounded hover:bg-blue-600 ml-80 mt-96"
                type="submit"
              >
                Save
              </button>

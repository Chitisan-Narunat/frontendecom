...
function Soundbars() {
  ...
  const addToCart = async (ProductId) => {
    try {
      ...
      const needAddress =
        message === "No Address" ||
        (typeof data !== "string" && data?.needAddress === true);
      ...
      const oid = Number(data?.orderId ?? data?.OrderId);
  ...
  const cat1 = products.filter((p) => (p.categoryId ?? p.CategoryId) === 3);

  const API_BASE =
    api.defaults.baseURL?.replace(/\/+$/, "") || "http://localhost:5283/api";
  useEffect(() => {
  ...
  return (
    <main className="min-h-screen bg-[#edeef0] dark:bg-[#0f1115] text-[#111827] dark:text-neutral-100 transition-colors">
      <Navbar
        ...
        cartCount={cartCount}
      />
      <div className="h-[1800px] flex items-start justify-center">
      ...
      {isLoginOpen && (
        <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-50">
          <div className="bg-white dark:bg-neutral-900 dark:text-neutral-100 rounded-lg shadow-lg p-6 w-96 relative">
            <button
              onClick={() => setIsLoginOpen(false)}
              className="absolute top-3 right-3 text-gray-600 dark:text-neutral-300 hover:text-black dark:hover:text-white"
            >
              <IoClose size={24} />
            </button>
            ...
            <form className="space-y-4">
              <input
                ...
                placeholder="Email"
                name="email"
                className="w-full border p-2 rounded bg-white dark:bg-neutral-800 border-gray-300 dark:border-neutral-700 placeholder-gray-400 dark:placeholder-neutral-400"
                value={form.email}
          ...
          <div className="bg-white dark:bg-neutral-900 dark:text-neutral-100 rounded-lg shadow-lg p-6 w-96 relative">
            ...
            <div className="space-y-4">
              <input
                ...
                className="w-full border p-2 rounded text-center tracking-widest bg-white dark:bg-neutral-800 border-gray-300 dark:border-neutral-700 placeholder-gray-400 dark:placeholder-neutral-400"
                value={otp}
                onChange={(e) => setOtp(e.target.value.replace(/\D/g, ""))}
              />
              <button
                disabled={isVerifying}
                className="w-full bg-gray-600 text-white py-2 rounded hover:bg-gray-800 disabled:opacity-50 transition"
              ...
              <button
                ...
                className="w-full border py-2 rounded disabled:opacity-50 bg-white dark:bg-neutral-800 border-gray-300 dark:border-neutral-700"
              ...
              <p className="text-[11px] text-center text-gray-400 dark:text-neutral-500 break-all">
                RequestId: {requestId}
              </p>
            </div>
          </div>
      ...
      {isMenuRendered && (
        <div
          ...
          className={`fixed inset-0 bg-black/50 flex justify-start items-stretch z-50 transition-opacity duration-300 ease-in-out ${
            isMenuVisible ? "opacity-100" : "opacity-0 pointer-events-none"
          }`}
        >
          <div
            onMouseDown={(e) => e.stopPropagation()}
            className={`bg-white dark:bg-neutral-900 dark:text-neutral-100 shadow-lg p-6 w-72 relative transform transition-transform duration-300 ease-in-out ${
              isMenuVisible ? "translate-x-0" : "-translate-x-full"
            }`}
          >
            <button
              onClick={() => setIsMenuOpen(false)}
              className="absolute top-6 left-9 text-gray-600 dark:text-neutral-300 hover:text-black dark:hover:text-white"
            >
              <IoClose size={24} />
            </button>
